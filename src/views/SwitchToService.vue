<template>
  ...
  >
    <v-toolbar flat>
      <v-toolbar-title>Change account type</v-toolbar-title>
    </v-toolbar>
...
export default {
  ...
  computed: {
    ...mapGetters(['id', 'terms', 'categoryTerms'])
  },
  mounted () {
    ...
    async next () {
      if (this.valid) {
        ...
        try {
          ...
          try {
            this.sId = this.id
            await this.$store.dispatch('convertToService', {
            ...
            this.$router.push(`service/${this.sId}`)
          } catch (e) {
            console.log(e)
          }
        } catch (e) {
